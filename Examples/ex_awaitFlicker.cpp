...
//
// ABOUT: fetch pictures from flicker
//
...
static FlickrPhotos parseFlickerResponse(streambuf& response)
{
...
static std::pair<std::string, std::string> makeFlickrQueryUrl(const std::vector<std::string>& tags, int perPage, int page)
{
    ...
    const std::string API_KEY = "4799aa95b2d8faa5a4e8cc7ff7f1034d";
...
static ut::AwaitableHandle asyncFlickerDownload(const std::vector<std::string>& tags, int numPics, int numPicsPerPage)
{
    static const int MAX_PARALLEL_DOWNLOADS = 6;

    return ut::startAsync("asyncFlickerDownload", [tags, numPics, numPicsPerPage](ut::Awaitable * /* awtSelf */) {
        struct DownloadSlot
        {
        ...
        try {
            ...
            while (totalPicsRemaining > 0) {
                ...
                // parse xml
                FlickrPhotos resp = parseFlickerResponse(response);
...
void ex_awaitFlicker()
{
    ...
    ut::AwaitableHandle awt = asyncFlickerDownload(splitTags, 25, 10);
